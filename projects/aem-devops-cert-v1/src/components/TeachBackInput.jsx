...
export default function TeachBackInput({ topics = [] }) {
  ...
  const handleSubmit = () => {
    ...
    // Simulate grading delay (placeholder — real grading via deposit skill)
    setTimeout(() => {
      setResults({
        logicAccuracy: 88,
        depth: 74,
        flow: 'Optimal',
        suggestions: [
          'Consider connecting core concepts to practical implementation scenarios.',
          'Your explanation structure is clear — add a concrete example for deeper scoring.',
        ],
        semanticGaps: [
          { name: 'Practical example', status: 'missing' },
          { name: 'Core definition', status: 'covered' },
          { name: 'Use case', status: 'covered' },
        ],
      })
      setGrading(false)
      setSubmitted(true)
